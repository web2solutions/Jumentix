...
16. Before every task, fetch/read current `main` and `dev`, inspect every requirement for changes, read `.agents/NFR-REGISTRY.md` and the canonical specification index, and update the per-task branch-check evidence in `.agents/AGENT-REGISTRY.md`; block work until complete (099).
