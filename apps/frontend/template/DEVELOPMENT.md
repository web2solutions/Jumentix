...
└── _nav.ts          # Navigation menu configuration
```

### Key Files

- **`App.vue`**: Main application component with router-view
- **`main.ts`**: Vue app creation, plugins, and mounting
- **`router/index.ts`**: Route definitions and configuration
- **`_nav.ts`**: Navigation menu structure for sidebar
...
**1. Create the page component** in `src/features/[feature]/`:

```vue
<!-- src/features/products/Products.vue -->
<template>
  <CRow>
    <CCol :xs="12">
      <CCard class="mb-4">
...
**3. Add navigation item to `src/_nav.ts`** (optional):
...
Add authentication logic in `router/index.ts`:
