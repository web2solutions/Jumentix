...
describe('shell views', () => {
  ...
  it('DefaultLayout mounts header, sidebar and footer and loads roles for the nav', async () => {
    ...
    expect(wrapper.find('.sidebar-nav').text()).toContain('Organizations');
    wrapper.unmount();
  });
  ...
  it('AppHeader exposes the localized top nav and the theme switch', async () => {
    ...
    await flush(1);
    const links = wrapper.findAll('.header-nav .nav-link').map((a) => a.text());
    expect(links).toContain('Painel');
    expect(links).toContain('Usuários');
