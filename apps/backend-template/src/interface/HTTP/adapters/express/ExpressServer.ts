...
class ExpressServer extends HTTPBaseServer<Express> {
  ...
  private createDocEndPoint() {
    this.application.use('/OASdoc', express.static('apps/backend-template/OASdoc'));
    this.application.use('/AsyncAPIdoc', express.static('apps/backend-template/AsyncAPIdoc'));
