...
describe('compileDatabaseClient', () => {
  ...
  it('returns in-memory client by default', () => {
    ...
    delete process.env.JUMENTIX_DATABASE_DRIVER;
  ...
  it('builds helper clients for relational/non-relational families', () => {
    expect.hasAssertions();
    process.env.JUMENTIX_DATABASE_CONNECTION_URL = 'postgres://aaa:aaa@127.0.0.1:5432/jumentix';
    process.env.JUMENTIX_DATABASE_CASSANDRA_CONTACT_POINTS = '127.0.0.1';
    process.env.JUMENTIX_DATABASE_PROJECT_ID = 'demo-project';
  ...
  it('applies env parsing fallbacks for firebase and rds dialect', async () => {
    ...
    process.env.JUMENTIX_FIREBASE_SERVICE_ACCOUNT_JSON = '{invalid-json';
    ...
    process.env.JUMENTIX_DATABASE_DIALECT = 'unknown-dialect';
    process.env.JUMENTIX_DATABASE_CONNECTION_URL = 'postgres://aaa:aaa@127.0.0.1:5432/jumentix';
    process.env.JUMENTIX_DATABASE_POOL_MAX = 'not-a-number';
    process.env.JUMENTIX_DATABASE_POOL_MIN = ' ';
    process.env.JUMENTIX_DATABASE_POOL_ACQUIRE_MS = 'invalid';
    expect(compileDatabaseClient()).not.toBe(InMemoryDbClient);
  });

  it('applies env parsing fallback for cassandra contact points and firebase non-object json', () => {
    expect.hasAssertions();
    process.env.JUMENTIX_DATABASE_DRIVER = 'cassandra';
    process.env.JUMENTIX_DATABASE_CASSANDRA_CONTACT_POINTS = ' , 127.0.0.1 , ';
    expect(compileDatabaseClient()).not.toBe(InMemoryDbClient);

    process.env.JUMENTIX_DATABASE_DRIVER = 'firebase';
    process.env.JUMENTIX_FIREBASE_SERVICE_ACCOUNT_JSON = '1';
    expect(compileDatabaseClient()).not.toBe(InMemoryDbClient);
  });
  ...
  it('covers parse fallbacks for numeric env and firebase json variants', () => {
    ...
    process.env.JUMENTIX_DATABASE_DRIVER = 'rds';
    process.env.JUMENTIX_DATABASE_CONNECTION_URL = 'postgres://aaa:aaa@127.0.0.1:5432/jumentix';
    process.env.JUMENTIX_DATABASE_POOL_MAX = 'NaN';
    expect(compileDatabaseClient()).not.toBe(InMemoryDbClient);

    process.env.JUMENTIX_DATABASE_DRIVER = 'firebase';
    process.env.JUMENTIX_FIREBASE_SERVICE_ACCOUNT_JSON = '"string-value"';
    expect(compileDatabaseClient()).not.toBe(InMemoryDbClient);
  });
});
