...
// Generated (gitignored) shell assets: they exist only after
// ci-cd/sync-service-management-cana-bundle.js runs, so the on-disk check
// below covers the committed files and the browser smoke covers these
// dynamically against the real server.
const GENERATED_SHELL_ASSETS = ['./vendor/cana/index.js'];

function committedShellAssets(assets: string[]): string[] {
  return assets.filter((asset) => asset !== './' && !GENERATED_SHELL_ASSETS.includes(asset));
}
...
describe('pwa shell service worker (JUM-489)', () => {
  ...
  it('precaches a shell that exists on disk and includes every boot-critical asset', () => {
    ...
    // vendored Cana bundle, JUM-484) are pinned dynamically by the browser
    // smoke, which requests EVERY entry against the real server.
    GENERATED_SHELL_ASSETS.forEach((generated) => {
      expect(sw.SHELL_ASSETS).toContain(generated);
    });
