...
/**
 * Moved out of `test/integration/Feathers/` by JUM-698.
 *
 ...
 *
 * It was not converted, and the reason is not effort: **neither
 * `@feathersjs/feathers` nor `@feathersjs/koa` is a dependency of this
 * repository.** `feathers/FeathersServer`
...
describe('feathers -> /localhost suite', () => {
  it('localhost should return 200 contract payload', async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      Context.run(new Map([['correlationId', 'feathers-correlation-id']]), () => {
        try {
    ...
    expect(response.json).toHaveBeenCalledWith({
      status: 'result',
      correlationId: 'feathers-correlation-id'
    });
  });
});
