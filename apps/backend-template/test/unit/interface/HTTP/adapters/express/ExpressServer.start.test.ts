...
describe('expressServer', () => {
  ...
  it('compiles as a singleton and serves docs and lifecycle probes', async () => {
    ...
    const [asyncApiRedirect, lifecycleProbe] = await Promise.all([
  ...
  it('allows CORS in dev and denies unknown origins in production', async () => {
    expect.hasAssertions();
    (process.env as { NODE_ENV?: string }).NODE_ENV = 'dev';
    delete process.env.JUMENTIX_CORS_ALLOWED_ORIGINS;
    ...
    const corsAllowedInDev = await request(application)
      .get('/express-lifecycle-probe')
      .set('Origin', 'https://allowed.example')
      .set('Authorization', 'Bearer unit-token');
    expect(corsAllowedInDev.status).toBe(200);
