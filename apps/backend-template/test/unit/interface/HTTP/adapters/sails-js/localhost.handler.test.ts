...
/**
 * Moved out of `test/integration/Sails-JS/` by JUM-698.
 *
 ...
 *
 * It was not converted, and the reason is not effort: **`sails` is not a
 * dependency of this repository.** `sails-js/SailsJsServer`
...
describe('sails-js -> /localhost suite', () => {
  it('localhost should return 200 contract payload', async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      Context.run(new Map([['correlationId', 'sails-correlation-id']]), () => {
        try {
    ...
    expect(response.json).toHaveBeenCalledWith({
      status: 'result',
      correlationId: 'sails-correlation-id'
    });
  });
});
