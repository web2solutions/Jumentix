...
/**
 * Moved out of `test/integration/Derby-JS/` by JUM-698.
 *
 ...
 *
 * It was not converted, and the reason is not effort: **`derby` is not a
 * dependency of this repository.** `derby-js/DerbyJsServer`
...
describe('derby-js -> /localhost suite', () => {
  it('localhost should return 200 contract payload', async () => {
    ...
    await new Promise<void>((resolve, reject) => {
      Context.run(new Map([['correlationId', 'derby-correlation-id']]), () => {
        try {
    ...
    expect(response.json).toHaveBeenCalledWith({
      status: 'result',
      correlationId: 'derby-correlation-id'
    });
  });
});
