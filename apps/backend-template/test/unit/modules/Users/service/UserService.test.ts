...
describe('user service', () => {
  ...
  it('uses read-through cache for getOneById/getAll and invalidates on write', async () => {
    expect.hasAssertions();
    ...
    expect(cacheService.getVersion).toHaveBeenCalledWith('users');
    expect(cacheService.set).toHaveBeenCalledWith(
      `users:v1:getOneById:${baseUser.id}`,
      expect.objectContaining({ id: baseUser.id })
    );
    expect(mockedGetUserById).toHaveBeenCalledTimes(1);

    await service.getAll({ username: 'john' }, { page: 1, size: 10 });
    expect(cacheService.set).toHaveBeenCalledWith(
      'users:v1:getAll:{"filters":{"username":"john"},"paging":{"page":1,"size":10}}',
      expect.any(Object)
    );
    expect(mockedGetAllUsers).toHaveBeenCalledTimes(1);

    await service.getAll({ ids: ['u1', 'u2'] as any }, { page: 1, size: 10 });

    await service.update(baseUser.id, { firstName: 'Mary' } as any);
    expect(cacheService.bumpVersion).toHaveBeenCalledWith('users');
