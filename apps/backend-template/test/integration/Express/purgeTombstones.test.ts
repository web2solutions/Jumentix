...
describe('express -> /internal/tombstones/purge', () => {
  beforeAll(async () => {
  ...
  it('dry-runs by default and does not require OAS auth', async () => {
    expect.hasAssertions();
    const response = await request(server)
      .post('/internal/tombstones/purge')
      .send({})
      .set('Accept', 'application/json');
    expect(response.statusCode).toBe(200);
    expect(response.body.dryRun).toBe(true);
    expect(response.body.olderThanDays).toBe(90);
  });
});
