<!--
Arquivo gerado automaticamente a partir de: apps/jumentix-website/documentation/README.md
Idioma alvo: Português (Brasil)
-->
# Índice de documentação do site Jumentix

Documentação técnica para aplicação do site comercial.

Contrato de público:

- O site deve priorizar conteúdo voltado a consumidores.
- Documentação de planejamento/migração e material interno de mantenedores deve permanecer em documentação de criadores/interna e não deve ser destacada como conteúdo principal do site.

## Índice

- [Auditoria de UX do AdonisJS e arquitetura de informação do Jumentix](./research/ADONISJS-UX-AUDIT.pt-BR.md)
- [Design system e Storybook](./DESIGN-SYSTEM-AND-STORYBOOK.pt-BR.md)
- [IA do site e plano de conversão](./WEBSITE-IA-AND-CONVERSION-PLAN.pt-BR.md)
- [Pipeline de conteúdo markdown](./CONTENT-PIPELINE.pt-BR.md)
- [Página de changelog](./CHANGELOG-PAGE.pt-BR.md)
- [SEO e baseline de performance](./SEO-AND-PERFORMANCE-BASELINE.pt-BR.md)
- [Deploy na Vercel](./VERCEL-DEPLOYMENT.pt-BR.md)
- [Integração NPM e Vercel](./NPM-AND-VERCEL-INTEGRATION.pt-BR.md)
