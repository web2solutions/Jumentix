---
title: "Domain Designer Features and Usage"
description: "Every Domain Designer capability and how to use it in practice."
---
...
Export OAS 3.1 now writes `x-services`, `servers[]` with `x-service-id`, `x-service` on operations and schemas, and `x-architecture-links`. The merged document is the Core view. The **OpenAPI** tab embeds Swagger UI (vendored from `apps/backend-template/OASdoc`). The service selector filters operations; Try it out uses that service's `servers` URL.
...
- `x-relations` rows carry `{ name, fromSchema, toSchema, fromCardinality, toCardinality }` — schema names, not model ids — and the importer restores
