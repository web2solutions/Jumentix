...
- **The wire contracts are linked, not duplicated.** The runtime-env API and
  ...
  Requirement 126, Contracts 1 and 1b,
...
- **The save is validated and confirmed.** Only write-allowlisted keys are
  ...
  post-write state the panel confirms (`Environment "staging" saved to .env.staging`).
...
- **Field validation** — `collectDeployTargetFieldIssues` in
  `deployTargetLifecycleValidation.js`,
...
- **Persistence and boot behaviour** — the `IDesignerStore` port and the
  ...
  belong to the
  [E3 document](/docs/jumentix/reference/service-management-module-architecture), and their
  user-facing promises to the
  [E6 document](/docs/jumentix/reference/service-management-cana-adoption).
...
- Ecosystem sources: `pm2/ecosystem.dev.cjs`, `pm2/ecosystem.staging.cjs`, `pm2/ecosystem.production.cjs`
