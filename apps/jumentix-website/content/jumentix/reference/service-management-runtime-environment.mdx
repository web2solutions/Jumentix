...
description: "Env-file locations, key tiers and the canonical enum sets the runtime API enforces."
---
...
- `JUMENTIX_HTTP_FRAMEWORK`
  ...
  - current supported values:
    ...
    - `adonis-js`
    - `total-js`
  - alias decision: `derby-js` and `sails-js` are the canonical spellings.
    The aliases `derby` and `sails` are NOT accepted — `RuntimeEnvironment.ts`
    fails fast on them. `hyper-express` was dropped from the runtime and
...
default `<repo-root>/pm2`) and returns `{ environment, fileName, path, exists, apps }`, where each app carries `{ name, script, interpreter, interpreterArgs, env, command }`. `command` is derived from the ecosystem definition —
...
- Unreadable or syntactically broken ecosystem file:
  `500 { "error": "PM2 ecosystem file operation failed.", "code", "path", "details" }`.
...
Management public-contracts requirement.
...
the Service Management public-contracts requirement, and the tier
...
Alias decision: `derby`/`derby-js` and `sails`/`sails-js` are the
...
- When `JUMENTIX_SERVICE_MANAGEMENT_AUTH_TOKEN` is set, `POST /api/runtime/env` requires `Authorization: Bearer <token>` and rejects
...
Responses distinguish parse, validation, and filesystem failures:
...
- Unsupported environment: `400 { "error": "Invalid environment request.", "details": … }` whose `details` name the unsupported value and the accepted
  list — on GET and POST alike; no file is written.
- Filesystem failures — missing env file (error code `ENV_FILE_NOT_FOUND`),
  permission errors, full disk: `500 { "error": "Environment file operation failed.", "code": …, "path": …, "details": … }`, where `code` is
