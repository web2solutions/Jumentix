...
(`sw.js`, a classic script served from
...
[Cana Usage Guide](/docs/jumentix/packages/cana/usage). This document states the boundary
...
- **The operations console and its status-surface contract** — the E5
  document, [Service Management Operations Console](/docs/jumentix/reference/service-management-operations-console);
...
- **The static-serving manifest rules** (boot-only vs on-miss) — documented
  in the application README; the
  precache↔static-manifest agreement is mentioned here only because the PWA
  tests pin it.

## References

- Token layer: `apps/service-management/tokens.css` (vendored; source of truth `components/design-system/tokens.css`), `styles.css`, `index.html`
- Accessibility behaviour: `src/ui/tabs.js`, `src/ui/canvas.js`, `src/ui/inspectors.js`, the global keyboard map in `script.js`; structural geometry pinned by `src/model/modelQueries.js`
- Storybook coverage: `components/service-management-designer/ServiceManagementDesigner.stories.tsx`, `scripts/storybook-smoke.mjs`, `.storybook/preview.tsx`
- PWA shell: `manifest.webmanifest`, `sw.js`, `src/pwa/pwaShell.js`, `icons/`, `server.js`
- Suites: `pwaShell.test.ts`, `pwaShell.browser.integration.test.ts`, `spaBoot.browser.integration.test.ts`, `modelQueries.test.ts`
- Requirements: 076 (EN/PT parity)
- Sibling E-chain documents: [Runtime Environment Contracts](/docs/jumentix/reference/service-management-runtime-environment) (E1), [Service Management Module Architecture](/docs/jumentix/reference/service-management-module-architecture) (E3), [Service Management Contract Parity Guarantees](/docs/jumentix/reference/service-management-contract-parity) (E4), [Service Management Operations Console](/docs/jumentix/reference/service-management-operations-console) (E5), [Cana Usage Guide](/docs/jumentix/packages/cana/usage), Design System and Storybook (website), Service Management Application
