...
description: "Consumer API guide hub for @jumentix/cana with focused subpages."
---
...
1. [Getting started](/docs/jumentix/packages/cana/usage/getting-started) — install Cana, create the `categories`
   and `tasks` tables, seed data, and read it back.
2. [Schema and keys](/docs/jumentix/packages/cana/usage/schema-keys) — versioned schema changes, inbound keys,
   generated keys, outbound keys and validation.
3. [Reading, writing and bulk operations](/docs/jumentix/packages/cana/usage/crud-bulk) — `get`, `add`, `put`,
   `update`, `delete`, `clear`, `bulkAdd`, `bulkPut` and predictable failure
   handling.
4. [Querying and plans](/docs/jumentix/packages/cana/usage/querying) — indexes, `equals`, `limit`, `offset`,
   `count`, `explain()` and complexity.
5. [Transactions and change events](/docs/jumentix/packages/cana/usage/transactions-events) — atomic writes,
   `CanaChangeEvent`, replay windows and UI store synchronization.
6. [Hooks and errors](/docs/jumentix/packages/cana/usage/hooks-errors) — `beforeWrite`, `afterCommit`,
   `CanaError` guards and common recovery branches.
7. [Storage and crash recovery](/docs/jumentix/packages/cana/usage/storage-recovery) — storage assessment,
   durability assessment, export/import and `resolveWrite()`.
8. [Workers and testing](/docs/jumentix/packages/cana/usage/workers-testing) — `createWorkerHost()`,
   `createRouter()`, `createWorkerClient()` and test strategy.
9. [API reference](/docs/jumentix/packages/cana/usage/api-reference) — compact method map and glossary.
