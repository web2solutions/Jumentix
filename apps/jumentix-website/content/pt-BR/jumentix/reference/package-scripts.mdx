...
| `tdd` | Use quando precisar desta operação específica do workspace. | `bun run tdd` | `NODE_ENV=dev bun x jest --watchAll` |
| `npm` | Use quando precisar desta operação específica do workspace. | `bun run npm` | `NODE_ENV=ci bun x jest ./apps/backend-template/test` |
| `test` | Executa a suíte padrão de testes do backend-template. | `bun run test` | `NODE_ENV=dev bun x jest ./apps/backend-template/test` |
