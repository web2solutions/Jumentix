---
title: "Paridade de Contrato do Service Management"
description: "O que cada exportação garante: OAS 3.1, AsyncAPI, proto, bundle de codegen e fidelidade de ida e volta."
---
...
estão em
[Funcionalidades e uso do Domain Designer](/docs/pt-BR/jumentix/reference/domain-designer-features)
(seções 10.1–10.2).
...
- **Códigos de erro canônicos.** As respostas de erro usam o conjunto de
  status de
  [ERROR-CONTRACTS-AND-RESPONSES](/docs/pt-BR/jumentix/reference/errors-responses)
...
- O que o OAS não consegue expressar nativamente atravessa como extensões
  ...
  `id`/`createdAt`/`updatedAt` do importador) e `x-field-flags: { pk, fk, unique }` por campo (emitido apenas quando os flags divergem da heurística
...
- **Relacionamentos atravessam como linhas `x-relations` de topo indexadas
  por nome de schema** (`{ name, fromSchema, toSchema, fromCardinality, toCardinality }`) — nunca por id de modelo, porque o importador recomputa
...
- **As suítes de unidade são a aplicação.** Todas as suítes nomeadas acima
  ...
  (`ci-cd/run-unit-tests.js`, que recusa uma
...
versionado de suíte completa: `{ kind: "service-management-suite", version: "2.0.0", domains, relationships, interfaces, serviceConfiguration, runtimeEnvironment, deployments, view }` — as mesmas seções que o documento
...
- Exportador/importador OAS: `packages/designer-core/src/exporters/designerExporters.js`, `designerImporters.js`
- Exportadores AsyncAPI/proto: `packages/designer-core/src/exporters/asyncApiExporters.js`; validador: `asyncApi30Validation.js`
- Codegen: `packages/designer-core/src/codegen/hexagonalCodegen.js`
- Validação de modelo / portão de exportação: `modelValidation.js`, `apps/service-management/script.js`
- Espelho RBAC: `src/model/rbacContract.js`; contrato: Contrato de autorização de tenant e RBAC
- Suítes: `designerRoundTrip.test.ts`, `designerPackageVersioning.test.ts`, `designerOasCompliance.test.ts`, `designerAsyncApiExport.test.ts`, `hexagonalCodegen.test.ts`, `rbacContract.test.ts`, `modelValidation.test.ts`
- Portões: `ci-cd/check-oas-route-resolution.js`, `ci-cd/check-hexagonal-boundaries.js`, `ci-cd/run-unit-tests.js`
- Alvos canônicos: `spec/1.0.0.yml`, `spec/asyncapi/`, `spec/asyncapi/1.0.0.grpc.yml`, `spec/asyncapi/async-api.proto`
- Requisitos: Requisito 036 (objetos de porta), Requisito 026 (conformidade de entidades OAS 3.1), Requisito 126, Contrato 3 (ownership e contratos públicos, Contratos 2–3)
- Documentos irmãos da cadeia E: Aplicação Service Management, [Arquitetura de módulos do Service Management e contrato da porta IDesignerStore](/docs/pt-BR/jumentix/reference/service-management-module-architecture), [Funcionalidades e uso do Domain Designer](/docs/pt-BR/jumentix/reference/domain-designer-features)
