---
title: "Contratos de Ambiente de Runtime"
description: "Locais dos arquivos de ambiente, níveis de chave e os enums canônicos que a API de runtime aplica."
---
...
### API de visualização do ecossistema PM2
...
onde cada app carrega `{ name, script, interpreter, interpreterArgs, env, command }`. `command` deriva da definição do ecossistema —
...
contratos públicos do Service Management.
...
de contratos públicos do Service Management, e as listas de níveis
...
Decisão de alias: `derby`/`derby-js` e `sails`/`sails-js` são o mesmo
...
- Quando `JUMENTIX_SERVICE_MANAGEMENT_AUTH_TOKEN` está definido, `POST /api/runtime/env` requer `Authorization: Bearer <token>` e rejeita qualquer
...
As respostas distinguem falhas de parse, validação e filesystem:
...
- Ambiente não suportado: `400 { "error": "Invalid environment request.", "details": … }` cujo `details` nomeia o valor não suportado e a lista de
  aceitos — tanto no GET quanto no POST; nenhum arquivo é escrito.
- Falhas de filesystem — arquivo env ausente (código de erro
  `ENV_FILE_NOT_FOUND`), erros de permissão, disco cheio: `500 { "error": "Environment file operation failed.", "code": …, "path": …, "details": … }`,
