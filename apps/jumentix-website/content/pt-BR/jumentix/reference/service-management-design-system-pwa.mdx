---
title: "Design System e PWA do Service Management"
description: "Tokens de design, o modelo de acessibilidade e como instalar, atualizar e recuperar o app shell."
---
...
website como
`components/service-management-designer/ServiceManagementDesigner.stories.tsx`,
...
sob `icons/`) e o service worker
(`sw.js`, um script clássico servido da
...
existe) e
`pwaShell.browser.integration.test.ts`,
...
[Guia de uso do Cana](/docs/pt-BR/jumentix/packages/cana/usage). Este documento estabelece
...
- **As regras do manifest de servir estático** (boot-only vs on-miss) —
  documentadas no README do aplicativo;
  a concordância pré-cache↔manifest estático é mencionada aqui apenas porque
  os testes de PWA a fixam.

## Referências

- Camada de tokens: `apps/service-management/tokens.css` (vendorizada; fonte da verdade `components/design-system/tokens.css`), `styles.css`, `index.html`
- Comportamento de acessibilidade: `src/ui/tabs.js`, `src/ui/canvas.js`, `src/ui/inspectors.js`, o mapa global de teclado em `script.js`; geometria estrutural fixada por `src/model/modelQueries.js`
- Cobertura Storybook: `components/service-management-designer/ServiceManagementDesigner.stories.tsx`, `scripts/storybook-smoke.mjs`, `.storybook/preview.tsx`
- Shell PWA: `manifest.webmanifest`, `sw.js`, `src/pwa/pwaShell.js`, `icons/`, `server.js`
- Suítes: `pwaShell.test.ts`, `pwaShell.browser.integration.test.ts`, `spaBoot.browser.integration.test.ts`, `modelQueries.test.ts`
- Requisitos: 076 (paridade EN/PT)
- Documentos irmãos da cadeia E: [Contratos de ambiente de runtime](/docs/pt-BR/jumentix/reference/service-management-runtime-environment) (E1), [Arquitetura de módulos do Service Management](/docs/pt-BR/jumentix/reference/service-management-module-architecture) (E3), [Garantias de paridade de contratos do Service Management](/docs/pt-BR/jumentix/reference/service-management-contract-parity) (E4), [Console de operações do Service Management](/docs/pt-BR/jumentix/reference/service-management-operations-console) (E5), [Guia de uso do Cana](/docs/pt-BR/jumentix/packages/cana/usage), Design System e Storybook (website), Aplicativo Service Management
