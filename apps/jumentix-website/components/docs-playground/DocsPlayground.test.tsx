...
describe('DocsPlayground catalogs', () => {
  ...
  it('jumentix browser lab demonstrates mutex rejection and dead-letter replay through controllers', () => {
    ...
    expect(snippet?.code).toContain('database.stores.tasks.add');
...
describe('DocsPlayground', () => {
  ...
  it('renders a canvas flow for the bulk mutex dead-letter playground with red rejected-flow semantics', () => {
    ...
    expect(screen.getByText(/starts in the React Context API component/i)).toBeInTheDocument();
    ...
    expect(screen.getByTestId('docs-playground-jumentix-browser-lab-bulk-mutex-dead-letter-cana-flow-canvas')).toBeInTheDocument();
    expect(screen.getByText('Cana + IndexedDB live state')).toBeInTheDocument();
    expect(screen.getByText(/real Cana events to animate adapter, client, transaction, Table API, IndexedDB, and subscribers/i)).toBeInTheDocument();
    expect(screen.getByTestId('docs-playground-jumentix-browser-lab-bulk-mutex-dead-letter-cana-flow-state')).toHaveTextContent('Cana events: 0');
  });
});
