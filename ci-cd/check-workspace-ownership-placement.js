...
function parseArgs(argv) {
  ...
  for (let i = 2; i < argv.length; i += 1) {
    ...
    if (arg === '--changed') {
      ...
      out.root = path.resolve(argv[++i] || process.cwd());
    }
  }
  return out;
}
 ...
 */
function inferAssertedWorkspaces(rel, contents, workspaces) {
  const asserted = new Set();
  const home = suiteHome(rel);

  // Path segment: ServiceManagement under a non-SM app → SM.
  if (/\/ServiceManagement\//.test(rel) && home !== 'apps/service-management') {
    asserted.add('apps/service-management');
  }

  // Tooling suites parked under backend-template.
  if (rel.startsWith('ci-cd/test/')) {
    asserted.add('ci-cd');
  }

  // Explicit path literals to other apps / packages / ci-cd.
  for (const ws of workspaces) {
    if (ws === home) continue;
    // Req 126 path pin: SM may resolve backend-template config by path only.
    if (home === 'apps/service-management' && ws === 'apps/backend-template') {
    ...
    if (new RegExp(`['"\`]${needle}(/|['"\`])`).test(contents)) {
      asserted.add(ws);
    }
  }

  // Relative requires of root ci-cd scripts from an app suite.
  if (home && home.startsWith('apps/') && /require\(['"](?:\.\.\/)+ci-cd\//.test(contents)) {
    asserted.add('ci-cd');
  }

  // Deep package src / jest.mock of packages/<P>/src from an app → dual home.
  const deepPkg = /packages\/([^/'"]+)\/src\//g;
  let match;
  while ((match = deepPkg.exec(contents)) !== null) {
    asserted.add(`packages/${match[1]}`);
  }
  const mockPkg = /jest\.mock\(\s*['"]packages\/([^/'"]+)\//g;
  while ((match = mockPkg.exec(contents)) !== null) {
    asserted.add(`packages/${match[1]}`);
  }

  // @src always means backend-template — OK when home is backend-template.
  if (/from ['"]@src\//.test(contents) || /require\(['"]@src\//.test(contents)) {
    if (home !== 'apps/backend-template') {
...
function loadAllowlist(root) {
  ...
  if (!Array.isArray(entries)) {
    throw new Error(`${ALLOWLIST_PATH} must be an array or { entries: [] }`);
  }
  return entries;
}
...
function collectSuites(root) {
  ...
  // Also scan ci-cd/test even before suiteRoots includes it (JUM-829).
  const extra = ['ci-cd/test'];
  const allRoots = [...new Set([...roots, ...extra])];
  return allRoots
...
function findViolations(root, options = {}) {
  ...
  for (const suite of suites) {
    ...
    for (const assertsWorkspace of asserted) {
      ...
      // Consumer wiring: app suite that only hits packages via @jumentix is OK —
      // inferAssertedWorkspaces does not add those. Path/deep-src hits remain.
  ...
  // Anti-regression (JUM-828): SM suites must not reappear under backend-template.
  const forbiddenHomes = [
  ...
  for (const home of forbiddenHomes) {
...
function run(root = process.cwd(), options = {}) {
  // Seed mode must ignore the existing allow-list so the file is a full
  // snapshot of current debt, not a delta against a stale snapshot.
