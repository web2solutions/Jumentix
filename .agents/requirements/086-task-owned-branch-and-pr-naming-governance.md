...
Every delivery boundary must execute the complete test matrix and must fail closed when any
required suite is missing, skipped, empty, aborted, or unsuccessful.
...
13. Every commit must run the full test matrix before the commit is accepted.
14. Every push must rerun the full test matrix before remote publication is accepted.
15. Every pull request, including `dev` to `main` promotion PRs, must run the full test matrix in CI and expose each required matrix result as auditable evidence.
...
19. Scope-aware optimization may change execution order or reuse verified artifacts, but it must not omit required matrix cells at commit, push, or PR boundaries.
...
6. Commit and push hooks require the repository's canonical full-matrix gate.
7. PR CI requires the same full-matrix contract and reports incomplete, missing, or failed cells as failure.
...
- Evidence includes the linked issue, task-owned branch, task-owned pull request to `dev`, `dev` to `main` promotion provenance when applicable, naming fields, full-matrix manifest, per-cell results, and validation logs.
