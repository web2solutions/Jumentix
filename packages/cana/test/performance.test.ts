 ...
 * ## What this is NOT
 *
 * `fake-indexeddb` is an in-memory implementation. Its constant factors are
 * nothing like a browser's disk-backed store, so **no absolute number here
 * transfers to production**. What does transfer is the shape: if an operation
...
describe('cana performance shape', () => {
  it('does not scale a limited query with table size', async () => {
    ...
    expect(largeTime).toBeLessThan(Math.max(smallTime * 4, 5));
    await small.close();
    await large.close();
  }, 60_000);

  it('does not scale an indexed lookup proportionally with table size', async () => {
    expect.hasAssertions();
    ...
    expect(largeTime).toBeLessThan(Math.max(smallTime * 25, 20));
    await small.close();
    await large.close();
  }, 60_000);
  ...
  it('counts without reading the rows', async () => {
    ...
    expect(countTime).toBeLessThan(readTime);
    await client.close();
  }, 60_000);

  it('does not scale a keyed get with table size', async () => {
    expect.hasAssertions();
  ...
  it('completes a bulk write of ten thousand rows', async () => {
    expect.hasAssertions();
    ...
    expect(result.keys).toHaveLength(LARGE);
    await expect(client.table<Row>('rows').count()).resolves.toBe(LARGE);
    await client.close();
  }, 120_000);

  it('applies a deep offset through the cursor rather than materialising', async () => {
    expect.hasAssertions();
    ...
    expect(deepPage).toBeLessThan(Math.max(earlyPage * 60, 60));
    await client.close();
  }, 60_000);
});
 ...
 *
 * `fake-indexeddb` is in-memory; a browser's IndexedDB is disk-backed with a
 * completely different cost profile. A millisecond threshold measured here would
 * be meaningless in production and flaky on a shared runner.
 *
