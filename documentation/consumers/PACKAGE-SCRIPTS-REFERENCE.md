...
| `test:integration:lambda` | Run tests for specific scope or profile. | `bun run test:integration:lambda` | `JUMENTIX_MESSAGE_MEDIATOR_ADAPTER=inmemory NODE_ENV=dev jest ./apps/backend-template/test/integration/Lambda --runInBand --coverage=false` |
...
| `docker:compose:service-template` | Start/stop containerized dependencies and services. | `bun run docker:compose:service-template` | `docker compose -f "apps/backend-template/docker-compose-service-templates.yml" --profile ${JUMENTIX_SERVICE_PROFILE:-rest} up -d --build` |
