...
- **Used with:** `@jumentix/cli-init` factory generator, guides (REST, realtime, SPA/PWA), public packages hub
- **Not responsible for:** private workspace tooling docs (config-*, agent-registry, security-scanner)
...
| **Factory CLI** | `@jumentix/cli-init` — `jumentix init|add|upgrade|doctor` generates a lean workspace for one factory mode. |
| **GenerationPlan** | Normalized plan from designer export, OAS, catalog URL, or Users preset before files are written. |
...
| --- | --- | --- |
| **Generate a product workspace** | You want a runnable Jumentix app (default first success) | `npx @jumentix/cli-init init` — see Step 3 |
| **Consume published packages** | You build an app that calls Jumentix SDKs only | `bun add @jumentix/sdk-rest-client` |
| **Work inside the monorepo** | You contribute to or extend the factory itself | `bun install` at the repo root |

For product teams generating a workspace (recommended):
...
**Success check:** factory path creates `my-app/` with `.jumentix/project.json`;
package path completes `bun install` without errors.

### Step 3 — First success: factory `init` (< 10 minutes)

The public factory generator is `@jumentix/cli-init`. It writes a lean Bun
workspace for one factory mode (`monolith` / `services` / `hybrid` / `frontend`)
from a GenerationPlan (`--preset=users`, `--from` designer/OAS/catalog).
...
ls apps          # core (+ frontend when hybrid)
```

What you should see:

- `.jumentix/project.json` — mode, plan, template cohort (not the retired `service-profile.json`)
- `.jumentix/manifest.json` — sha256 of generated files
- `jumentix.init.json` — answers for `--config` round-trips
- `apps/*` workspace packages ready for `bun run dev|test|lint|build`

Extend later with `jumentix add domain|service|frontend`, refresh templates with
`jumentix upgrade`, and diagnose with `jumentix doctor`. Modes map to the
[service factory capabilities matrix](/docs/jumentix/concepts/architecture)
(see also repository doc `JUMENTIX-SERVICE-FACTORY-CAPABILITIES-MATRIX.md`).

**Success check:** `demo/.jumentix/project.json` exists, `doctor` exits `0`, and
you can name the HTTP adapter folder under `apps/`.

Then open the [REST API guide](/docs/jumentix/guides/rest-api) and call the
generated service.
...
### Step 5 — Touch Cana in the browser (< 10 minutes)

Before building a full offline PWA, confirm the docs toolchain works. Run the
playground below — **Run** should return a green result, then **Reset** should
...
| `jumentix init` exits 1 on missing answers | `--non-interactive` without full flags/config | Pass `--preset`/`--mode`/`--http`/`--db` or a `jumentix.init.json` via `--config` | Init writes `.jumentix/project.json` |
| Cannot find adapters in generated app | Looking in the monorepo seed instead of the generated tree | `cd` into the `init` output and open `apps/<service>` | You can name one HTTP adapter file |
...
- [ ] Run `jumentix init` (or `npx @jumentix/cli-init init`) and locate `.jumentix/project.json`.
