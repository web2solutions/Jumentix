...
| `docker:up:mongodb` | Sobe/derruba dependências e serviços em containers. | `bun run docker:up:mongodb` | `docker compose -f "apps/backend-template/docker-compose-mongodb.yml" down --remove-orphans \|\| true && docker compose -f "apps/backend-template/docker-compose-mongodb.yml" up -d --build --wait --force-recreate --remove-orphans` |
