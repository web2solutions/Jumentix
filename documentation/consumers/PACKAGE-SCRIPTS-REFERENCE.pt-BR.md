...
| `agent-registry:register` | Registra ou atualiza o agente atual no registry do Firestore. | `bun run agent-registry:register` | `bun ci-cd/agent-registry-cli.js register` |
| `agent-registry:heartbeat` | Atualiza heartbeat e refs de branch do agente no Firestore. | `bun run agent-registry:heartbeat` | `bun ci-cd/agent-registry-cli.js heartbeat` |
| `agent-registry:assign` | Atribui o agente a uma tarefa e épico do Linear no Firestore. | `bun run agent-registry:assign` | `bun ci-cd/agent-registry-cli.js assign` |
| `agent-registry:complete` | Marca a tarefa atual como concluída no Firestore. | `bun run agent-registry:complete` | `bun ci-cd/agent-registry-cli.js complete` |
| `agent-registry:sync` | Grava o snapshot local do Firestore em `.agents/registry-snapshot.json`. | `bun run agent-registry:sync` | `bun ci-cd/agent-registry-cli.js sync` |
| `agent-registry:check` | Valida o snapshot local contra o Firestore. | `bun run agent-registry:check` | `bun ci-cd/agent-registry-cli.js check` |
...
| `test:integration:lambda` | Roda testes para escopo ou perfil específico. | `bun run test:integration:lambda` | `JUMENTIX_MESSAGE_MEDIATOR_ADAPTER=inmemory NODE_ENV=dev jest ./apps/backend-template/test/integration/Lambda --runInBand --coverage=false` |
