...
The Feathers adapter connects the Feathers runtime to Jumentix HTTP contracts. It stays at the edge: receive the request, normalize input, call use cases, and map the result back to a response.

## Integrated technology

Service-oriented Node.js framework for REST and realtime service endpoints.
...
Use it for: Service APIs that may expose the same behavior through HTTP and realtime channels.

## When to avoid

Avoid when you only need a narrow REST edge and no service framework conventions.
