...
The LoopBack adapter connects the LoopBack runtime to Jumentix HTTP contracts. It stays at the edge: receive the request, normalize input, call use cases, and map the result back to a response.

## Integrated technology

API framework with strong model, controller, and OpenAPI conventions.
...
Use it for: Model-heavy APIs that benefit from convention-driven controllers and OpenAPI metadata.

## When to avoid

Avoid when the service should stay intentionally thin at the HTTP edge.
...
const adapterProfile = {
  ...
  entrypoint: 'apps/backend-template/src/interface/HTTP/adapters/loopback/loopback.ts'
} as const;
