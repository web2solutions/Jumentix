...
The Restify adapter connects the Restify runtime to Jumentix HTTP contracts. It stays at the edge: receive the request, normalize input, call use cases, and map the result back to a response.

## Integrated technology

REST-focused Node.js framework designed for service APIs and explicit request/response handling.
...
Use it for: Service APIs where explicit REST semantics and operational clarity matter more than ecosystem size.

## When to avoid

Avoid for UI-heavy server apps or projects that expect the Express middleware ecosystem.
