...
The Cloudflare Workers adapter connects the Cloudflare Workers runtime to Jumentix HTTP contracts. It stays at the edge: receive the request, normalize input, call use cases, and map the result back to a response.

## Integrated technology

Edge runtime based on the Web Fetch API, deployed close to users through Cloudflare.

- **Runtime model:** Edge Fetch handler
- **Jumentix contract:** handlers call controllers/use cases without leaking framework types into the domain.

## When to use

Use it for: Low-latency edge APIs, auth gateways, and browser-aligned request handling.

## When to avoid

Avoid when the adapter needs unrestricted Node.js APIs or long server processes.
