...
O adapter AWS Lambda conecta o runtime AWS Lambda aos contratos HTTP do Jumentix. Ele fica na borda: recebe request, normaliza entrada, chama use cases e transforma o resultado em response.

## Tecnologia integrada

Funções serverless da AWS invocadas por API Gateway, EventBridge, filas ou chamadas diretas Lambda.

- **Modelo de runtime:** Invocação serverless por função
- **Contrato Jumentix:** handlers chamam controllers/use cases sem vazar tipos do framework para o domínio.

## Quando usar

Use quando: APIs orientadas a eventos, workloads com picos e times que já operam na AWS.

## Quando evitar

Evite quando cold starts, paridade local ou conexões long-lived dominam o workload.
