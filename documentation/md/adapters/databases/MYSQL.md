...
The MySQL adapter connects the Jumentix persistence contract to MySQL. Use cases keep talking to repository ports; database selection stays in composition.

## Integrated technology

MySQL through the SQL client profile

- **Data model:** Relational SQL
- **Jumentix driver:** `MySQL`
- **Runtime selection:** `JUMENTIX_DATABASE_DRIVER=MySQL`

## When to use

Use it for: Common relational workloads, SaaS CRUD, and teams with MySQL operational experience.

## When to avoid

Avoid when advanced PostgreSQL-specific features are required.
...
The example above shows the full contract with an in-memory implementation so it can be read end to end. In production, composition injects the real client selected by `JUMENTIX_DATABASE_DRIVER=MySQL`; the domain stays the same.
