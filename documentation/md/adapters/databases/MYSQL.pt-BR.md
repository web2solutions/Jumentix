...
O adapter MySQL conecta o contrato de persistência do Jumentix à tecnologia MySQL. Use cases continuam falando com portas de repositório; a escolha de banco fica na composição.

## Tecnologia integrada

MySQL pelo perfil de cliente SQL

- **Modelo de dados:** SQL relacional
- **Driver Jumentix:** `MySQL`
- **Seleção de runtime:** `JUMENTIX_DATABASE_DRIVER=MySQL`

## Quando usar

Use quando: Workloads relacionais comuns, CRUD SaaS e times com experiência operacional em MySQL.

## Quando evitar

Evite quando recursos avançados específicos de PostgreSQL são obrigatórios.
...
JUMENTIX_DATABASE_DRIVER=MySQL bun run smoke:db:mysql
```
...
const adapterProfile = {
  ...
  smokeTest: 'bun run smoke:db:mysql'
} as const;
...
O exemplo acima mostra o contrato completo com uma implementação em memória para ser lido de ponta a ponta. Em produção, a composição injeta o cliente real selecionado por `JUMENTIX_DATABASE_DRIVER=MySQL`; o domínio continua igual.
