...
The Aurora adapter connects the Jumentix persistence contract to Aurora. Use cases keep talking to repository ports; database selection stays in composition.

## Integrated technology

Amazon Aurora-compatible relational profile

- **Data model:** Managed relational SQL
- **Jumentix driver:** `Aurora`
- **Runtime selection:** `JUMENTIX_DATABASE_DRIVER=Aurora`

## When to use

Use it for: AWS relational workloads needing managed operations and cloud scaling paths.

## When to avoid

Avoid when the app must stay cloud-agnostic at the database layer.
...
The example above shows the full contract with an in-memory implementation so it can be read end to end. In production, composition injects the real client selected by `JUMENTIX_DATABASE_DRIVER=Aurora`; the domain stays the same.
