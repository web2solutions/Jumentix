...
The SQLite adapter connects the Jumentix persistence contract to SQLite. Use cases keep talking to repository ports; database selection stays in composition.

## Integrated technology

Embedded SQL database file

- **Data model:** Embedded relational SQL
- **Jumentix driver:** `SQLite`
- **Runtime selection:** `JUMENTIX_DATABASE_DRIVER=SQLite`

## When to use

Use it for: Local development, edge prototypes, tests, and single-node utilities.

## When to avoid

Avoid as the primary database for horizontally scaled write-heavy services.
...
The example above shows the full contract with an in-memory implementation so it can be read end to end. In production, composition injects the real client selected by `JUMENTIX_DATABASE_DRIVER=SQLite`; the domain stays the same.
