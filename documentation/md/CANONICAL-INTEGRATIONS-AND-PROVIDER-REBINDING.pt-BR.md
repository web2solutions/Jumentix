...
| --- | --- | --- |
| CI | Workflow CircleCI canônico deste repositório | Executor canônico das PRs para `dev`, promoções para `main` e branches protegidas enquanto GitHub Actions billing bloqueia execução hospedada |
| Cobertura | Job CircleCI de cobertura, artefatos JSON/LCOV e limites de projeto e patch | Autoridade canônica; publicação Codecov é visibilidade |
| Publicação Codecov | Upload pelo Codecov CLI no CircleCI com `CODECOV_TOKEN` | Espelho de dashboard após os thresholds próprios passarem |
...
| Segredos | Scanner OSS fixado executado pelo CircleCI | Substituto próprio para checks pagos em PR |
...
- **GitHub Actions billing bloqueado:** workflows hospedados de Actions estão
  desabilitados neste repositório até a cobrança voltar a permitir execução.
  CircleCI e os mesmos comandos Bun locais fazem os gates.
- **Publicação Codecov restaurada:** CircleCI envia LCOV pelo Codecov CLI após
...
1. Jobs CircleCI usam ferramentas fixadas, dependências congeladas e evidência fail-closed.
...
5. A proteção de branch exige somente checks determinísticos dos jobs CircleCI.
