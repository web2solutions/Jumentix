...
### Dependency scanning covers the installed Bun tree

`bun run deps:audit` resolves the packages installed from `bun.lock`, submits
them to the OSV.dev batch API, retrieves advisory details, and fails closed when
the service is unavailable or returns an invalid response. Accepted risks are
explicitly identified with expiry dates in the scanner source.

The scanner is repository-owned and runs directly in `ci:gate`; it requires no
provider token, generated compatibility lockfile, webhook, or GitHub App.
