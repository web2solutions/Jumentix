...
| Static `ci:gate` checks (JUM-903) | Hosted CI never runs the `ci:gate` script, so every step listed only there ran nowhere; `ci:gate:static` groups the cheap ones — dependency overrides, OSV `deps:audit`, HTTP adapter authenticity, commit authorship, package suites, build freshness, CLI template freshness, website content routes, RTDB indexes, CI provider, OAS relations | `package.json` `ci:gate:static`, `ci-cd/run-branch-quality-gate.js`, `ci-cd/test/ci-gate-coverage.test.ts` | preflight of every branch-gate path; the coverage suite fails when a step is added to `ci:gate` without a CI path. `frontend:coverage:check` runs in the CircleCI `coverage` job |
