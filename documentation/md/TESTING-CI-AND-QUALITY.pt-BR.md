...
| Checks estáticos do `ci:gate` (JUM-903) | O CI hospedado nunca executa o script `ci:gate`, então todo passo listado só nele não rodava em lugar nenhum; `ci:gate:static` agrupa os baratos — overrides de dependência, `deps:audit` (OSV), autenticidade dos adapters HTTP, autoria de commits, suítes de pacote, freshness de build, freshness de templates da CLI, rotas de conteúdo do site, índices RTDB, provedor de CI, relações OAS | `package.json` `ci:gate:static`, `ci-cd/run-branch-quality-gate.js`, `ci-cd/test/ci-gate-coverage.test.ts` | preflight de todo caminho do branch gate; a suíte de cobertura falha quando um passo entra no `ci:gate` sem caminho de CI. `frontend:coverage:check` roda no job `coverage` da CircleCI |
