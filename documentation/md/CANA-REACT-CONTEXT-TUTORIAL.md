...
`TasksProvider` is the only component that knows how Cana events become React
state. Leaf components stay boring: they call `addTask` and `toggleTask`, then
render `state.categories`, `state.tasks`, and `state.events`.
