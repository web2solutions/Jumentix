...
5. Commit, push, and pull-request boundaries must execute the destination-appropriate
   canonical gate: unit tests for `dev`, and the full matrix for `main`. Smoke-only,
   docs-only, missing, empty, skipped, or unreported results are not delivery evidence.
