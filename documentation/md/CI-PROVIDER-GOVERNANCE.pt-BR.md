...
Execução manual da matriz completa (Requisito 113): `curl -X POST https://circleci.com/api/v2/project/gh/web2solutions/Jumentix/pipeline -H "Circle-Token: ..." -d '{"branch":"dev","parameters":{"force_full":true}}'` — o parâmetro de pipeline `force_full` mapeia para o contexto `scheduled-full` e executa sob demanda a matriz completa de jobs.
