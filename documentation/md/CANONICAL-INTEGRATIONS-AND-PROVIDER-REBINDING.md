...
| SonarQube Cloud | Organization `xpertminds`, project `XpertMinds_Jumentix`, GitHub App authorization, and `SONAR_TOKEN` are active; the canonical quality gate passed with 100% new-code coverage and zero new issues | Observe the same fail-closed scan on the next canonical task PR |
| Snyk | Organization `XpertMinds` copied legacy settings, integrations, and policies; its GitHub App is authorized for all repositories; the `XpertMinds/Jumentix` import created the package projects; and a new 90-day token is stored in the `secrets` environment | Observe a terminal `security/snyk` PR check |
...
| GitGuardian | The GitHub App is authorized for all XpertMinds repositories and the XpertMinds source is linked with automatic history scanning and blocking PR checks enabled | Observe a terminal `GitGuardian Security Checks` result on the canonical PR |
| Cursor Bugbot | XpertMinds shows 5/5 repositories enabled, including both Jumentix repositories, with Bugbot triggered on every push | Observe a terminal Cursor Bugbot result on the canonical PR |
