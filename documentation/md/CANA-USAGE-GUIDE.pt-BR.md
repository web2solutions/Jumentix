...
- **Responsável por:** IndexedDB (com fallback explícito localStorage)
- **Usado com:** designer-core e guia SPA/PWA
...
20. [Checklist júnior](#20-checklist-júnior-eu-consigo--)
21. [Erros comuns (referência rápida)](#21-erros-comuns-referência-rápida)
22. [Próximo passo](#22-próximo-passo)
...
const one = await tasks.get(1);   // Task | undefined
```

Para tipar também a chave:

```ts
const tasks = client.table<Task, number>('tasks');
```
...
```ts
// v1
{ version: 1, stores: [{ name: 'tasks', keyPath: 'id' }] }
...
await client.table('tasks').add({ id: 1, title: 'a' });
...
await tasks.get(1);                          // Task | undefined
await tasks.add({ id: 1, title: 'first' });   // falha se a chave existir
await tasks.put({ id: 1, title: 'replaced' }); // insere ou substitui
await tasks.update(1, { title: 'renamed' });  // mescla em um registro existente
...
```ts
await tasks.bulkAdd([{ id: 1, title: 'a' }, { id: 2, title: 'b' }]);
await tasks.bulkPut([{ id: 1, title: 'changed' }, { id: 3, title: 'new' }]);
await tasks.bulkDelete([1, 2]);
```
...
await tasks.query({ index: 'byCategory', equals: 'work' });

// Intervalo
await tasks.query({ index: 'byUpdatedAt', range: { lower: start, upper: end } });

// Intervalo semiaberto
await tasks.query({ index: 'byUpdatedAt', range: { lower: start, upperOpen: true } });

// Ordem inversa, limitada
await tasks.query({ index: 'byUpdatedAt', direction: 'prev', limit: 20 });

// Paginação
await tasks.query({ index: 'byUpdatedAt', offset: 40, limit: 20 });

// Somente valores únicos
await tasks.query({ index: 'byCategory', distinct: true });
```
...
```ts
await tasks.count();                                   // tudo
await tasks.count({ index: 'byCategory', equals: 'work' }); // correspondentes
```
...
```ts
const { records, plan } = await tasks.explain({ index: 'byCategory', equals: 'work' });

plan.usedIndex             // 'byCategory'
plan.fullScan              // false
...
```ts
it('não faz varredura completa na tabela tasks', async () => {
  const { plan } = await tasks.explain({ index: 'byCategory', equals: currentCategory });
...
```ts
const recent = await tasks.query({ index: 'byUpdatedAt', range: { lower: since } });
const workTasks = recent.filter((task) => task.categoryId === 'work');
```
...
const { outcome, result, events, correlationId, attemptedAt } =
  await client.transaction('readwrite', ['categories', 'tasks'], async (scope) => {
    const now = Date.now();
    await scope.table('categories').put({ id: 'work', name: 'Work' });
    const task = {
      id: 9,
      title: 'Publicar quadro offline de tasks',
      categoryId: 'work',
      completed: false,
      updatedAt: now
    };
    await scope.table<Task>('tasks').add(task);
    return task;
  });
```
...
```ts
// ERRADO
await client.transaction('readwrite', ['tasks'], async (scope) => {
  const remote = await fetch('/api/tasks/1');   // ← a transação termina aqui
  await scope.table('tasks').put(await remote.json());  // TransactionInactive
});
```

```ts
// CERTO — busque primeiro, depois abra a transação
const remote = await (await fetch('/api/tasks/1')).json();
await client.transaction('readwrite', ['tasks'], async (scope) => {
  await scope.table('tasks').put(remote);
});
```
...
```ts
await client.transaction('readwrite', ['tasks'], async (scope) => {
  await scope.table('tasks').add({ id: 1, title: 'a' });
...
const client = createClient({
  ...
  schema,
  hooks: {
    beforeWrite: (context) => ({ ...(context.record as Task), updatedAt: Date.now() }),
...
```ts
const client = createClient({ name: 'tasks-app', schema, operationLedger: true });
```
...
const compilers = buildDatabaseClientCompilers<IDatabaseClient>({
  ...
  indexedDbClient: () => createCanaDatabaseClient({ name: 'tasks-app', schema })
});
```

Depois selecione como qualquer outro driver — `JUMENTIX_DATABASE_DRIVER=IndexedDB`, ou
...
```ts
const database = createCanaDatabaseClient({ name: 'tasks-app', schema });

await database.connect();               // abre
database.stores.tasks                 // CanaTable, chaveado pelos nomes do schema
...
const rows = await router.send({ kind: 'query', store: 'tasks' });
```
...
function testClient() {
  ...
  return createClient({ name: 'tasks-app', schema, factory: new IDBFactory() });
}
```
...
| Termo | Significado |
| --- | --- |
| **Client** | Retorno de `createClient` — ponto de entrada para `open`, tabelas, transações e assinaturas. |
| **Store** | Um object store no schema (como uma tabela); nomeado em `schema.stores`. |
| **Versão do schema** | Inteiro `schema.version`; deve aumentar para o IndexedDB aplicar mudanças estruturais. |
| **Chave inbound** | Chave dentro do registro via `keyPath`. |
| **Chave outbound** | Chave passada como segundo argumento de `add`/`put`. |
| **Escopo da transação** | Lista de nomes de stores em `transaction`; tocar outros falha. |
| **Evento de mudança** | `CanaChangeEvent` emitido só após escrita confirmada (`created`, `updated`, `deleted`, `cleared`). |
| **Desfecho de escrita** | `committed`, `rolled-back` ou `unknown` — veja §12 para recuperação de falhas. |
| **Livro de operações** | Store opcional que grava ids de operação na mesma transação dos dados (`operationLedger: true`). |
| **Avaliação de durabilidade** | `durabilityAssessment()` — saúde do armazenamento local para o usuário (`lost`, `at-risk`, `best-effort`, `durable`). |

---

## 20. Checklist júnior (“Eu consigo …”)

- [ ] Criar client, definir schema, chamar `open()` e adicionar um registro.
- [ ] Explicar por que `open()` é explícito e o que acontece se eu pular.
- [ ] Escolher `add` vs `put` vs `update` para um cenário dado.
- [ ] Executar `query` limitada com índice e confirmar com `explain()` que `fullScan` é false.
- [ ] Escrever transação multi-store sem await de trabalho não-IndexedDB dentro do escopo.
- [ ] Assinar eventos de mudança e ignorar minhas escritas com `originId`.
- [ ] Tratar `QuotaExceeded` e `TransactionInactive` com `isCanaErrorCode`.
- [ ] Exportar com `exportAll()` e descrever quando avisar o usuário sobre armazenamento `at-risk`.

---

## 21. Erros comuns (referência rápida)

Condensado de §10 e §17. Para contexto completo, siga os links das seções.

| Sintoma | Causa | Correção | Verificar sucesso |
| --- | --- | --- | --- |
| `InvalidRequest` antes de qualquer leitura | Tabela chamada antes de `await client.open()` | Aguarde `open()` uma vez na inicialização | `get`/`add` funcionam |
| Mudança de schema sem efeito | Não aumentou `schema.version` | Aumente a versão e reabra | Novo store/índice aparece no DevTools |
| `TransactionInactive` | `await fetch` ou trabalho não-IDB dentro da transação | Mova awaits externos para fora; abra nova transação | Escrita confirma com `outcome: 'committed'` |
| `UpgradeBlocked` | Outra aba segura versão antiga do banco | Feche ou recarregue a outra aba | `open()` funciona na nova tentativa |
| `QuotaExceeded` | Armazenamento do browser cheio | Libere espaço; exporte dados críticos antes | `durabilityAssessment().level` melhora |
| Assinante perdeu mudanças | `sinceCursor` fora da janela retida | Recarga completa; reassine sem cursor obsoleto | Eventos aplicam sem lacuna |
| `resolveWrite` → `unresolvable` | Livro desligado ou versão do schema não aumentada ao ligar livro | Ative `operationLedger` e aumente versão do schema | `committed` ou `rolled-back` definitivo |

---

## 22. Próximo passo

Combine persistência offline com validação de domínio em
[designer-core](/docs/pt-BR/jumentix/packages/designer-core/usage) e siga o
